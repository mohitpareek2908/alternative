...
function Registration() {
	...
	**/
	...
	this.resendOtp = (req,res,next,callback)=>{
		...
		}else if(page == KID_PROFILE_SETTNIG_VIEW_OTP)
		{
			userConditions["validate_string"] = validateString; 
		}
		else{
		...
		Registration.getUserData(req,res,options).then(response=>{
			...
			getRandomOTP().then(otp=>{
				...
				{
					updateData = {
						paternal_verification_code : otp
					}
				}
	...
	this.logOut = (req, res, next)=>{
		...
		//	let slug	 		= 	(req.body.slug) 		?	req.body.slug		:"";
			...
			let userIsActive = (loginUserData.active) 		?	loginUserData.active 		:DEACTIVE;
	...
	*/
    this.forgotPassword = function (req, res, next, callback) {
        ...
        users.findOne($match, {}, (err, result) => {
			...
			let is_deleted = result.is_deleted ? result.is_deleted : DEACTIVE;
			let is_email_verified = result.is_email_verified ? result.is_email_verified : DEACTIVE;

			if( is_email_verified == DEACTIVE)
			{
				finalResponse = {
                    'data': {
                        status: STATUS_ERROR,
                        result: {},
                        message: res.__("Your email is not verified.Please verify.")
                    }
                };
                return returnApiResult(req,res,finalResponse);
	...
	this.resetPassword = (req,res,next)=>{
		...
		let options 	= 	{
			conditions	:	conditions,
			fields		:	{_id :1,full_name:1,email:1,active:1}
		};
	...
	/**
	 * Function for stopping all push notification 
	 *
	 * @param req	As Request Data
	 * @param res	As Response Data
	 * @param next	As Callback argument to the middleware function
	 ...
	 * @return json
	 **/
	 this.stopAllPn = function (req, res, next, callback) {

		/** Sanitize Data **/
		req.body 				=	sanitizeData(req.body,NOT_ALLOWED_TAGS_XSS);
		let loginUserData 	    =	(req.user_data) 		?	req.user_data 			:	"";
        let userId			    =	(loginUserData._id)		?	loginUserData._id		:	"";		
		let stopAllPn			= 	(req.body.stop_all_pn)	?	req.body.stop_all_pn	: DEACTIVE;
				...
				, (error, updateResult) => {
					...
					var finalResponse = {
						'data': {
							...
							message: res.__("front.system.pn_all_pn_stoped_successfully"),
						}
					};
					return returnApiResult(req,res,finalResponse);
				...
				, (error, updateResult) => {
					...
					var finalResponse = {
						'data': {
							...
							message: res.__("front.system.pn_all_pn_started_successfully"),
						}
					};
					return returnApiResult(req,res,finalResponse);
	...
	/**
	 * Function for stopping push notification 
	 *
	 * @param req	As Request Data
	 * @param res	As Response Data
	 * @param next	As Callback argument to the middleware function
	 ...
	 * @return json
	 **/
	this.stopPn = function (req, res, next, callback) {

		/** Sanitize Data **/
		req.body 				=	sanitizeData(req.body,NOT_ALLOWED_TAGS_XSS);
		let loginUserData 	    =	(req.user_data) 		?	req.user_data 			:	"";
        let userId			    =	(loginUserData._id)		?	loginUserData._id		:	"";		
		let pnStop				= 	(req.body.pn_stop)		?	req.body.pn_stop		: "";
		let pnRemove			= 	(req.body.pn_remove)	?	req.body.pn_remove		: DEACTIVE;
		let stopAllPn			= 	(req.body.stop_all_pn)	?	req.body.stop_all_pn	: DEACTIVE;
				...
				, (error, updateResult) => {
					...
					var finalResponse = {
						'data': {
							...
							message: res.__("front.system.pn_stoped_successfully"),
						}
					};
					return returnApiResult(req,res,finalResponse);
   ...
   */
                    ...
                    function (err, result) {
                        ...
                        if (result) {
							...
							if(page == FORGOT_PASSWORD_PAGE_TYPE){
								...
								if(otp != result.paternal_verification_code) {
									errMessageArray.push({'param':'otp','msg':res.__("user.incorrect_otp_message")});
								}
							}
							else{
                            ...
                            if(page == VERIFY_ACCOUNT_PAGE_TYPE){
								...
								msg= "front.user.email_address_verified";
							}else if(page == FORGOT_PASSWORD_PAGE_TYPE){
								var updateData = {
									modified: getUtcDate(),
									forgot_password_opt_code: "",
								}
								msg= "front.user.otp_verified";
							}else if(page == KID_PROFILE_SETTNIG_VIEW_OTP)
							{		
								var updateData = {
									modified: getUtcDate(),
									paternal_verification_code: "",
									validate_string: "",
								}
								msg= "front.user.paternal_control_verified";
                            ...
                            users.updateOne({
								...
								getUserDetailBySlug(req, res, userOptions).then(response => {
									...
									let userFullName 	= (response.result.full_name) ? response.result.full_name : "";
									let slug 			= (response.result.slug) ? response.result.slug : "";
									...
									if(page == VERIFY_ACCOUNT_PAGE_TYPE){
											if(userEmail != ""){
												...
												let emailOptions = {
													to: userEmail,
													action: "uzube_welcome_mail",
													rep_array: [userFullName]
												};
												/** Send email **/
												sendMail(req, res, emailOptions);
											}
									...
									var finalResponse = {
										'data': {
											...
											token_life		: 	JWT_CONFIG.tokenLife,
											message: res.__(msg, response.result.full_name),
										}
									};
									 return returnApiResult(req,res,finalResponse);
								});
     ...
     * */
		...
		//	req.body 			= sanitizeData(req.body, NOT_ALLOWED_TAGS_XSS);
			...
			let id 				= userId;
			let email 			= (req.body.new_email) ? req.body.new_email : '';
			...
			switch (editType) {
				case "email":
					...
					getRandomOTP().then(email_otp => {
						...
						let userFullName 			= (loginUserData.full_name) ? loginUserData.full_name : "";
						let userOldEmail 			= (loginUserData.email) ? loginUserData.email : "";
						let userNewEmail 			= (req.body.new_email) ? req.body.new_email.toLowerCase() : '';
						let currentTimeStamp 		= new Date().getTime();
							...
							users.updateOne(
								...
								},
								{
									$set: {
										...
										mobile_verification_code		: email_otp,
										otp_verification_code_time		: nextDay,
										modified						: getUtcDate(),
								...
								/** Set options for send email ***/
									let emailOptions = {
										...
										rep_array: [userFullName, userNewEmail,email_otp]
									};
