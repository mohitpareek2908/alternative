			...
			try{
				...
				users.findOne({
					...
					if(!userErr && userResult){
						...
						if (CORPORATES_MODULES_LIST.constructor === Array) {
							corporateModules = CORPORATES_MODULES_LIST.map(function (records) {
								return (records) ? ObjectId(records) : "";
							});
						}
						let conditionsArray = {
						...
						if(userRoleId == SUPER_ADMIN_ROLE_ID){
							conditionsArray['_id']	=	{ $nin:corporateModules };
						}
						const admin_modules	= db.collection('admin_modules');
						admin_modules.aggregate([
							{$match	 : conditionsArray},
							{$lookup : {
								"from" 			: "admin_modules",
								"localField"	: "parent_id",
								"foreignField"	: "_id",
