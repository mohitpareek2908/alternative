function NewsletterSubscribers(){
	...
	*/
			...
			configDatatable(req,res,null).then(dataTableConfig=>{
				...
				async.parallel([
					(callback)=>{
						...
						collection.find(dataTableConfig.conditions,{_id:1,email:1,status:1,is_subscribe:1,modified:1,subscriber_type:1}).collation(COLLATION_VALUE).sort(dataTableConfig.sort_conditions).limit(limit).skip(skip).toArray((err, result)=>{
							callback(err, result);
						});
					},
					(callback)=>{
	...
	this.addSubscriber = (req,res,next)=>{
		if(isPost(req)){
			...
			const newsletter_subscribers = db.collection(TABLE_NEWSLETTER_SUBSCRIBERS);
			newsletter_subscribers.findOne({
				...
				newsletter_subscribers.insertOne({
					...
					req.flash(STATUS_SUCCESS,res.__("admin.newsletter.newsletter_subscriber_has_been_added_successfully"));
					res.send({
	...
	* 
	...
	this.deleteSubscriber = (req,res)=>{
		...
		/** Delete subscriber **/
		const subscribers = db.collection(TABLE_NEWSLETTER_SUBSCRIBERS);
