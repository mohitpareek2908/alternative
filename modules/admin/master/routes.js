...
app.all(modulePath+":type/add",checkLoggedInAdmin,addMasterValidationRules(),validate,(req,res,next) => {
	var adminMaster = require(modelPath);
	adminMaster.addMaster(req,res,next);
});

/** Routing is used to edit master **/
app.all(modulePath+":type/edit/:id",checkLoggedInAdmin,editMasterValidationRules(),validate,(req,res,next) => {
	var adminMaster = require(modelPath);
