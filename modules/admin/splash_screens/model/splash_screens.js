...
function SplashScreens(){
    ...
    this.getScreensItemList = (req, res)=>{
		...
		if(isPost(req)){
			...
			const collection	= 	db.collection(TABLE_SPLASH_SCREENS);
				...
				async.parallel([
					(callback)=>{
						...
						collection.find(dataTableConfig.conditions,{_id:1,image:1,is_active:1,is_deleted:1,modified:1,title:1}).collation(COLLATION_VALUE).sort(dataTableConfig.sort_conditions).limit(limit).skip(skip).toArray((err, result)=>{
							...
							let options = {
								"file_url" 			: 	SPLASH_SCREENS_IMG_URL,
								"file_path" 		: 	SPLASH_SCREENS_FILE_PATH,
								"result" 			: 	result,
			...
			/** render listing page **/
			req.breadcrumbs(BREADCRUMBS["admin/splash_screens/list"]);
			res.render("list",{
	...
	this.addScreen = (req,res,next)=>{
		...
		if(isPost(req)){
			...
			let fileType	 		= (req.body.post_media_type)? req.body.post_media_type	: "";
			...
			if(errMessageArray.length == 0){
				...
				let options	=	{
					'image' 	:	image,
					'filePath' 	: 	SPLASH_SCREENS_FILE_PATH
				};

				consoleLog("fileType is "+fileType);
				if(fileType == POST_TYPE_VIDEO)
				{
				...
				moveUploadedFile(req, res,options).then(response=>{
					...
					splashscreens.insertOne({
						...
						req.flash(STATUS_SUCCESS,res.__("admin.splash_screens.splash_screens_has_been_added_successfully"));
						res.send({
							status			: STATUS_SUCCESS,
							redirect_url	: WEBSITE_ADMIN_URL+'splash_screens',
							message			: res.__("admin.splash_screens.splash_screens_has_been_added_successfully")
						});
					});
				}).catch(next);
			}else{
			...
			getLanguages().then((languageList)=>{
				...
				req.breadcrumbs(BREADCRUMBS['admin/splash_screens/add']);
				res.render('add',{
		...
		if(isPost(req)){
				...
				moveUploadedFile(req, res,options).then(response=>{
					...
					splashscreens.updateOne({
						_id	:	ObjectId(splashScreencreenId)
							...
							/** Send success response */
							req.flash(STATUS_SUCCESS,res.__("admin.splash_screens.splash_screens_has_been_updated_successfully"));
							res.send({
								status			: STATUS_SUCCESS,
								redirect_url	: WEBSITE_ADMIN_URL+'splash_screens',
								message			: res.__("admin.splash_screens.splash_screens_has_been_updated_successfully")
							});
						});
				}).catch(next);
			...
			getLanguages().then((languageList)=>{
				getSplashScreenDetails(req, res).then(response=>{
						...
						req.breadcrumbs(BREADCRUMBS['admin/splash_screens/edit']);
	...
	let getSplashScreenDetails = (req,res)=>{
		return new Promise(resolve=>{
			...
			if(!splashScreenId || splashScreenId ==""){
				...
				try{
					const splash_screens	= db.collection("splash_screens");
	...
	this.deleteScreen = (req,res)=>{
			...
			if(splashScreenId){
				/** Delete user*/
				const splash_screens = db.collection(TABLE_SPLASH_SCREENS);
				splash_screens.updateOne({
					...
					if(!err){	
						/** Send success response **/
						req.flash(STATUS_SUCCESS,res.__("admin.splash_screens.splash_screen_deleted_successfully"));
						res.redirect(WEBSITE_ADMIN_URL+"splash_screens");
					}else{
						/** Send error response **/
		...
		if(splashScreenId && (statusType && (statusType == ACTIVE_INACTIVE_STATUS))){
			try{
				...
				splash_screens.update({_id : ObjectId(splashScreenId)},{$set :updateData},(err,result)=>{
					if(!err){
						/** Send success response **/
						req.flash(STATUS_SUCCESS,res.__("admin.splash_screens.splash_screens_status_has_been_updated_successfully"));
						res.redirect(WEBSITE_ADMIN_URL+"splash_screens");
					}else{
						/** Send error response **/
						req.flash(STATUS_ERROR,res.__("admin.system.something_going_wrong_please_try_again"));
...
module.exports = new SplashScreens();
