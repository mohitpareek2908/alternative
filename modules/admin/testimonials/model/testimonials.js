...
function Testimonials(){
	 ...
	 * @return render/json
	 */
    this.getTestimonialList = (req, res)=>{
			...
			configDatatable(req,res,null).then(dataTableConfig=>{
				...
				async.parallel([
					(callback)=>{
						...
						collection.find(dataTableConfig.conditions,{_id:1,image:1,is_active:1,modified:1,name:1,description:1}).collation(COLLATION_VALUE).sort(dataTableConfig.sort_conditions).limit(limit).skip(skip).toArray((err, result)=>{
							/** Set options for append image **/
							let options = {
								"file_url" 			: 	TESTIMONIAL_SCREENS_IMG_URL,
								"file_path" 		: 	TESTIMONIAL_SCREENS_FILE_PATH,
								"result" 			: 	result,
	...
	this.addScreen = (req,res,next)=>{
		...
		if(isPost(req)){
			...
			let screenTitle 		= (req.body.name)? req.body.name	: "";
			...
			if(errMessageArray.length == 0){
				...
				let options	=	{
					'image' 	:	image, 
					'filePath' 	: 	TESTIMONIAL_SCREENS_FILE_PATH
				};
				...
				moveUploadedFile(req, res,options).then(response=>{
					...
					const splashscreens = db.collection(TABLE_TESTIMONIALS);
					splashscreens.insertOne({
						...
						req.flash(STATUS_SUCCESS,res.__("admin.testimonial.testimonial_has_been_added_successfully"));
						res.send({
				...
				moveUploadedFile(req, res,options).then(response=>{
					...
					splashscreens.updateOne({
						_id	:	ObjectId(splashScreencreenId)
							...
							/** Send success response */
							req.flash(STATUS_SUCCESS,res.__("admin.testimonial.testimonial_details_has_been_updated_successfully"));
							res.send({
								status			: STATUS_SUCCESS,
								redirect_url	: WEBSITE_ADMIN_URL+'testimonials',
								message			: res.__("admin.testimonial.testimonial_details_has_been_updated_successfully")
							});
						});
				}).catch(next);
	...
	let getSplashScreenDetails = (req,res)=>{
		return new Promise(resolve=>{
			...
			if(!splashScreenId || splashScreenId ==""){
				...
				try{
					const splash_screens	= db.collection(TABLE_TESTIMONIALS);
		...
		if(splashScreenId && (statusType && (statusType == ACTIVE_INACTIVE_STATUS))){
			try{
				...
				splash_screens.update({_id : ObjectId(splashScreenId)},{$set :updateData},(err,result)=>{
					if(!err){
						/** Send success response **/
						req.flash(STATUS_SUCCESS,res.__("admin.testimonial.testimonial_status_has_been_updated_successfully"));
    ...
    this.viewTestimonial = (req, res, next)=>{
		...
		getSplashScreenDetails(req, res,next).then(response=>{
			...
			/** Render edit page  **/
			req.breadcrumbs(BREADCRUMBS['admin/testimonials/view']);
			res.render('view',{
				result : (response.result) ? response.result :{}
			});
		}).catch(next);
