function Setting() {
	...
	let writeSettingDetails = (req,res)=>{
		return new Promise(resolve=>{
			...
			console.log(frontUrl+FRONT_SRC_FILE_PATH);
			...
			settings.find({},{projection: {_id:1,key_value:1,value:1}}).toArray((err, result)=>{
				if(!err){
					...
					fs.writeFile(WEBSITE_ROOT_PATH+"config/settings.json", JSON.stringify(settingsObj), "utf8",function(err){
						fs.writeFile(frontUrl+"trixxie/src/settings.json", JSON.stringify(settingsObj), "utf8",function(err){
							
							fs.writeFile(frontUrl+"trixxie/dist/settings.json", JSON.stringify(settingsObj), "utf8",function(err){
