...
function PushNotification() {
	...
	this.updateNotificationsDetails = (req,res,next)=>{
		...
		if(isPost(req)){
			...
			}else if(userType==ALL_RIDERS){
				conditions['user_role_id']	=	RIDER_USER_ROLE_ID;
				
			}else if(userType==ALL_LOGIN_DRIVERS){
				conditions['user_role_id']	=	DRIVER_USER_ROLE_ID;
				conditions['is_online']		=	ACTIVE;
				
			}else if(userType==ALL_LOGIN_RIDERS){
				conditions['user_role_id']	=	RIDER_USER_ROLE_ID;
				conditions['is_online']		=	ACTIVE;
			}
			...
			req.checkBody({				
				'user_type': {
					notEmpty	: true,
					errorMessage: res.__("admin.push_notification.please_select_user_type")
				},
				'message': {
					notEmpty	: true,
					errorMessage: res.__("admin.push_notification.please_enter_message")
				},
			});
	...
	this.getUserListUserTypeWiseAjax	= 	(req,res)=>{
		...
		consoleLog("value of userType is ");
		consoleLog(userType);
		
		if(userType==ALL_KIDS){			
			conditions['user_role_id']		=	KIDS_USER_ROLE_ID;
			
		}else if(userType==ALL_TEENS){
			conditions['user_role_id']		=	TEENS_USER_ROLE_ID;
			
		}else if(userType==ALL_ADULTS){
			conditions['user_role_id']		=	ADULTS_USER_ROLE_ID;
