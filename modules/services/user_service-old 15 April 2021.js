	...
	* */
	this.addUser = (req, res, next, callback) => {
		return new Promise(resolve=>{
			...
			var phoneNumber   	= (req.body.mobile_number) ? req.body.mobile_number : "";
		...
		//	let userId			=	(req.body.user_id) 			? 	req.body.user_id	:ObjectId();	
			...
			consoleLog(req.body);
				...
				moveUploadedFile(req, res,BannerImageOptions).then(bannerResponse=>{
					...
					consoleLog("Here it is ");
					if (/\s/.test(fullName)) {
						// It has any kind of whitespace
						consoleLog(/\s/.test(fullName));
						consoleLog("Conatns Space");
					}else{
						consoleLog(/\s/.test(fullName));
						consoleLog("NOP");
				...
				let lastName 	= 	((/\s/.test(fullName))=="false") ? "" : fullNameBreak[1];
					...
					var imageName 	= 	(response.fileName) ? response.fileName 	:"";
					var bannerImageName 	= 	(bannerResponse.fileName) ? bannerResponse.fileName 	:"";

					consoleLog("authUserId IS ");
					consoleLog(authUserId);
